...
generate_commentary( stock_data_tbl, user_input = "AAPL")

# 6.0 TEST WORKFLOW ----
get_stock_list("DAX")
"ADS.DE, Adidas" %>%
  get_symbol_from_user_input() %>%
  get_stock_data( from = "2020-06-01", to = "2021-01-12", mavg_short = 5, mavg_long = 8) %>%
  # plot_stock_data() %>% 
  generate_commentary(user_input = "ADS.DE")
